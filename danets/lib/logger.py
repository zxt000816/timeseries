...
class Train_Log():
    def __init__(self, logname, best_model_path, resume_dir=None):
        time_str = datetime.now().strftime("%m-%d_%H%M")
        if resume_dir:
            self.resume_dir = os.path.join('./danets_logs', resume_dir)
            self.log_dir = self.resume_dir
        else:
            self.log_dir = os.path.join('./danets_logs/',  logname + '_' +time_str)
